// Ported from RikkaApps/Shizuku (manager/src/main/java/moe/shizuku/manager/adb/AdbKey.kt),
// licensed under Apache License 2.0. See THIRD_PARTY_NOTICES.md.
// Adapted: package renamed; rikka.core.ktx.unsafeLazy -> stdlib lazy(NONE).
...
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    val sslContext: SSLContext by lazy(LazyThreadSafetyMode.NONE) {
        // 플랫폼 기본 SSLContext("TLSv1.3", 프로바이더 미지정) 대신 org.conscrypt:conscrypt-android
        // 를 명시적으로 써야 한다 — SPAKE2 페어링에 필요한 TLS exporter(RFC 5705,
        // Conscrypt.exportKeyingMaterial)가 안드로이드 플랫폼 내장 Conscrypt(com.android.org.conscrypt)
        // 에선 hidden API라 공개 SDK로 컴파일이 안 되는데, 같은 걸 하는 오픈소스 Conscrypt를 별도
        // 프로바이더로 등록하면 공개 API(org.conscrypt.Conscrypt)로 동일 기능을 쓸 수 있다.
        val sslContext = SSLContext.getInstance("TLSv1.3", org.conscrypt.Conscrypt.newProvider())
